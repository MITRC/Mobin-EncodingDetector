...
import ir.mitrc.htmlchardet.Charsets;
import ir.mitrc.htmlchardet.HTMLCharsetDetector;
		...
		while (langURLQueue.size() != 0 && !isStop) {
			try {
				...
				if (Charsets.isValid(charset)) {
					...
					updateDetectedCharsetStat(charset, detectedCharset, Evaluation.mobinHTMLCharDetStat);
